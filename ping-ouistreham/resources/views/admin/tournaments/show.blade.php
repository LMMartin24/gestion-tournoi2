                    ...
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @forelse($slot->subTables as $series)
                            <div class="bg-black/40 border border-white/10 p-6 rounded-2xl relative group hover:border-indigo-500/50 transition-all">
                                ...
                                {{-- Dans ta boucle des séries --}}
                                <div class="mt-6 pt-4 border-t border-white/5 flex justify-between items-center">
                                    {{-- NOUVEAU BOUTON --}}
                                    <a href="{{ route('admin.sub-tables.participants', $series->id) }}" 
                                    class="text-indigo-400 hover:text-white text-[10px] font-black uppercase italic transition flex items-center gap-2">
                                        ...
                                        Voir les inscrits ({{ $series->registrations->count() }})
                                    </a>

                                    <form action="{{ route('admin.sub_tables.destroy', $series->id) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button class="text-red-500/30 hover:text-red-500 text-[9px] font-black uppercase italic transition">Supprimer</button>
                                    </form>
                                </div>
                            </div>
                        @empty
